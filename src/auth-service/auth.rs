...
impl Auth for AuthService {
    async fn sign_in(
        ...
        let result: Option<String> = self.users_service.lock().unwrap().get_user_uuid(req.username, req.password); // Get user's uuid from `users_service`. Panic if the lock is poisoned.
        ...
        let session_token: String = self.sessions_service.lock().unwrap().create_session(&user_uuid); // Create new session using `sessions_service`. Panic if the lock is poisoned.
