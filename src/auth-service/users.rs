...
impl Users for UsersImpl {
    ...
    fn get_user_uuid(&self, username: String, password: String) -> Option<String> {
        let user: &User = match self.username_to_user.get(&username) { // Retrieve `User` or return `None` is user can't be found.
            Some(user) => user,
            None => return None,
        };
    ...
    fn delete_user(&mut self, user_uuid: String) {
        self.uuid_to_user.remove(&user_uuid).and_then(|user| {
