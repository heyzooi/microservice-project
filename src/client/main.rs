...
use tonic::{Request, Response, IntoRequest};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let auth_ip = env::var("AUTH_SERVICE_IP").unwrap_or("localhost".to_owned());
    let mut client: AuthClient<Channel> = AuthClient::connect(format!("http://{auth_ip}:50051")).await?; // Create new `AuthClient` instance. Propagate any errors.

    let cli = Cli::parse();

    match &cli.command {
        Some(Commands::SignIn { username, password }) => {
            let request: Request<SignInRequest> = SignInRequest {
                username: username.to_owned(),
                password: password.to_owned(),
            }.into_request(); // Create a new `SignInRequest`.
        ...
        Some(Commands::SignUp { username, password }) => {
            let request: Request<SignUpRequest> = SignUpRequest {
                username: username.to_owned(),
                password: password.to_owned(),
            }.into_request(); // Create a new `SignUpRequest`.
        ...
        Some(Commands::SignOut { session_token }) => {
            let request: Request<SignOutRequest> = SignOutRequest {
                session_token: session_token.to_owned(),
            }.into_request(); // Create a new `SignOutRequest`.
